...
function governanceSettings() {
  return [
    ...
    },
    {
      ...
      valueType: "number",
      enabled: true,
      description: `${appMeta.title}高优先级工单的完成时限，单位为小时。`,
      updatedBy: "演示管理员",
    },
    {
      group: "通知设置",
      ...
      data: [
        {
          ...
          action: "初始化导入",
          actor: item.owner,
          content: `来自${item.sourceType}《${item.sourceTitle}》：${record.modelType}已创建，渠道${record.industry}，项目${record.region}，等待处理。`,
        },
        {
          modelId: item.id,
          sourceType: item.sourceType,
          ...
          action: "智能客服建议",
          actor: "智能客服助手",
          content: `来自${item.sourceType}《${item.sourceTitle}》：已完成首轮意图识别，风险等级${item.riskLevel}，建议继续回访确认。`,
        },
      ],
    });
