...
function localBusinessDraft({ prompt, item }: GlmAssistantInput): GlmAssistantResponse {
  ...
  return {
    ...
    result: [
      ...
      `${focusAreas[0]}：建议围绕"${item.category}"先核验${item.sourceBatch}中的历史会话和FAQ匹配结果。`,
      `${focusAreas[1]}：结合${item.description}，补充标准答复、回访话术和预约提醒建议。`,
      `${focusAreas[2]}：对高风险工单增加转人工和升级处理节点，降低投诉风险。`,
    ...
    const response = await fetch(GLM_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `你是${appMeta.department}${appMeta.shortName}的智能客服助手。请围绕${appMeta.aiExperience.focusAreas.join("、")}输出结构化服务建议，包括客户答复、回访话术、预约提醒和风险处理，避免技术细节和接口术语。`,
          },
          {
            role: "user",
            content: [
              `工单主题：${input.item.title}`,
              `工单类型：${input.item.category}`,
              ...
              `历史上下文：${historyText}`,
              `本次诉求：${input.prompt}`,
              `请以"工单主题 / 客户意图 / 处理建议 / 回访话术 / 风险提醒"五段输出。`,
            ].join("\n"),
          },
        ],
        temperature: 0.3,
