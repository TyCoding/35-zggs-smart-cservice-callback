...
export async function createWorkItem(input: WorkItemInput) {
  ...
  const item = await prisma.investModel.create({
    data: {
      ...
      industry: input.industry?.trim() || "人工录入",
      region: input.region?.trim() || "未分配项目",
      ...
      inputParams: input.inputParams?.trim() || input.description,
      outputResult: input.outputResult?.trim() || "待补充处理结果",
      prediction: input.prediction?.trim() || "待补充后续建议",
      source: "控制台录入",
      sourceType: appMeta.sourceObjectName,
      sourceTitle: input.title,
      sourceBatch: `${appMeta.dataSourceTitle}·人工登记`,
  ...
  await prisma.modelEvent.create({
    data: {
      ...
      action: "创建客服工单",
      actor: input.owner,
      content: `来自${item.sourceType}《${item.sourceTitle}》：${input.category}已创建，客户诉求与处理信息已同步至服务台。`,
    },
  });
  ...
  await recordAuditLog({
    ...
    actor: input.owner,
    summary: `${item.modelType}已创建并同步处理人。`,
  });
...
export async function advanceWorkItem(id: string) {
  ...
  await prisma.modelEvent.create({
    data: {
      ...
      action: "状态推进",
      actor: item.owner,
      content: `来自${item.sourceType}《${item.sourceTitle}》：工单状态由${item.status}推进至${status}。`,
    },
  });
  ...
  await recordAuditLog({
    ...
    targetName: item.title,
    actor: item.owner,
    summary: `${item.modelType}状态由${item.status}推进至${status}。`,
  });
